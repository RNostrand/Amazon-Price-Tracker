...
def transformProduct(soup):
    ...
    try:
        stars = soup.find('div', id='averageCustomerReviews').find('span', class_='a-icon-alt').text.strip()
    except:
        stars = 'Stars Unavailable'
   
    try:
        ratings = soup.find('div', id='averageCustomerReviews').find('span', id='acrCustomerReviewText').text.strip()
    except:
